...
test("formal manifest is fixed-membership, deterministic, and CRLF-stable", async () => {
  ...
  try {
    ...
    await writeFile(unexpected, "# Not a baseline artifact\n", "utf8");
    const membershipFailure = spawnSync(process.execPath, [script, "--repo", fx.root], {
      encoding: "utf8",
      windowsHide: true,
    });
    assert.notEqual(membershipFailure.status, 0);
    assert.match(membershipFailure.stderr, /formal artifact membership mismatch/);
  } finally {
    await fx.cleanup();
  }
});
