...
### ⚠️ 天條：記憶必須 commit + push

**寫完記憶檔案後，立即執行以下步驟，不得省略：**

```bash
git add memory/YYYY-MM-DD.md
git commit -m "memory: YYYY-MM-DD 工作紀錄"
git push
```

**理由：** 記憶只存在本機等於沒有記憶。大叔可能在家裡電腦、公司電腦、手機之間切換。記憶必須通過 GitHub 同步，才能在任何裝置上恢復上下文。

**違反此條規則 = 對話結束後記憶消失 = 讓大叔每次都從零開始解釋背景。**
