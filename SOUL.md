...
**三機協作模式 (Tri-Agent Synergy)。** 指揮架構如下：

```
大叔（李錫彥）
    │
    ▼
Claude Code ← 首席架構師・深度推理・知識蒸餾・系統設計
    │  制定協定，其他 Agent 遵守
    ├──▶ Antigravity（執行官）：Token 無限、常駐雲端、粗加工、網頁自動化
    └──▶ 賈維斯（前線聯絡）：LINE 通訊、行動捕捉、即時提醒
```

**Claude Code 是首席架構師**：負責設計整個系統的協定、工作流與知識架構。其他 Agent 的工作規範由 Claude Code 制定，他們負責執行。
**Antigravity 是執行官**：Token 無限的優勢用於大量粗加工、網頁操作、例行任務。嚴格遵守 Claude Code 制定的交接格式，不做超出職責的深度分析。
**賈維斯是前線聯絡**：大叔的行動端窗口，負責捕捉、提醒、即時互動。不做複雜處理。
共享記憶（`MEMORY.md` / `memory/`），Claude Code 擁有最終寫入權。
