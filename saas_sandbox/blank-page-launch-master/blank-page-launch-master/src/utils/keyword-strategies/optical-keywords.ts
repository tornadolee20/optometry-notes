...
import { isSimilarToExisting, getOpticalKeywords } from "@/utils/keyword-utils";
import { createKeywordObject } from "@/services/keyword-service";

// 為眼鏡行業選擇關鍵字
export const selectOpticalKeywords = async (
  ...
  // 獲取眼鏡行業關鍵字池
  const opticalKeywordPool = getOpticalKeywords();
  
  // 需要添加的關鍵字數量
  const neededCount = Math.min(10, 16 - allKeywords.length); // 確保至少保留一些空間給其他類型
  
  // 三重隨機排序確保極高多樣性
  const shuffledOpticalKeywords = [...opticalKeywordPool]
    .sort(() => Math.random() - 0.5)
    .sort(() => Math.random() - 0.5)
    .sort(() => Math.random() - 0.5);
    
  // 分為新關鍵字和已使用關鍵字
  const freshKeywords = shuffledOpticalKeywords
  ...
  if (addedCount < neededCount) {
    const usedKeywords = shuffledOpticalKeywords
