-- Fix ambiguous column reference in admin_grant_free_subscription function
...
BEGIN
  ...
  DO UPDATE SET
    ...
    expires_at = GREATEST(
      COALESCE(store_subscriptions.expires_at, '-infinity'),
      COALESCE(store_subscriptions.trial_ends_at, '-infinity'),
      timezone('utc', now())
