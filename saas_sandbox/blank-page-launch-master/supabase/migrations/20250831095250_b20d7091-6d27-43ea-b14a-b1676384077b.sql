-- Update the admin_grant_free_subscription function to better handle conflicts
...
BEGIN
  ...
  -- 檢查是否已有訂閱記錄
  SELECT EXISTS(SELECT 1 FROM public.store_subscriptions WHERE store_subscriptions.store_id = _store_id)
  ...
  IF _subscription_exists THEN
    ...
    SET 
      plan_type = _plan,
      status = CASE WHEN _plan = 'trial' THEN 'trial' ELSE 'active' END,
    ...
    RETURNING store_subscriptions.store_id, store_subscriptions.plan_type, store_subscriptions.status, store_subscriptions.expires_at, store_subscriptions.trial_ends_at, store_subscriptions.auto_renew
