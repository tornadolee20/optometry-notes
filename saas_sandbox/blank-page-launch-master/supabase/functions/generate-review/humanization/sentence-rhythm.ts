...
export function insertAbruptSentences(text: string): string {
  if (!text || Math.random() > 0.25) return text; // 25% chance
  
  // Categorized by mood for variety
  const abruptByMood = {
    chill: ['還行。', '可以。', '不差。', '就很 OK。'],
    earnest: ['值得一提。', '印象深刻。', '這點很加分。', '真心推薦。'],
    emotional: ['真的！', '超讚。', '感動。', '太棒了。', '很可以！'],
    neutral: ['真的。', '沒錯。', '挺好的。', '很棒。', '讚。'],
  };
  
  // Pick a random mood category
  const moods = Object.keys(abruptByMood) as (keyof typeof abruptByMood)[];
  const mood = moods[Math.floor(Math.random() * moods.length)];
  const pool = abruptByMood[mood];
  ...
  const abrupt = pool[Math.floor(Math.random() * pool.length)];
