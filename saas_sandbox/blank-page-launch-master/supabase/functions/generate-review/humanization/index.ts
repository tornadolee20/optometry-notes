...
export function generateHumanizationElements(config: HumanizationConfig): HumanizationResult {
  ...
  // 3. Natural interjections (mood can be overridden later by caller)
...
export function validateHumanizedReview(
  ...
  if (!isWithinFlexibleRange(wordCount, isHumanized, keywordCount)) {
    issues.push(`Word count ${wordCount} outside expected range`);
  }
