...
export const useStoreListData = () => {
  ...
  const fetchStores = useCallback(async () => {
    ...
    try {
      const { data: storesData, error: storesError } = await supabase
        .from('stores')
        .select('id, store_name, email, phone, address, store_number, status, industry, created_at, line_id, notes, tags')
  ...
  const handleBatchStatusUpdate = async (newStatus: StoreStatus) => {
    ...
    try {
      ...
      toast({
        variant: 'destructive',
        title: '錯誤',
        description: err instanceof Error ? err.message : '批量更新失敗'
      });
    }
  };
  ...
  const handleSingleStatusUpdate = async (storeId: string, newStatus: StoreStatus) => {
    try {
      const { error } = await supabase
        .from('stores')
        .update({ status: newStatus })
