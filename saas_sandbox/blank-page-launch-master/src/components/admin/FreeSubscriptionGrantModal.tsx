...
export const FreeSubscriptionGrantModal = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onSuccess?.();
      onClose();
      setForm({ duration: '', reason: '', notes: '', autoRenew: false });
    } catch (error) {
        ...
        <div className="space-y-4">
          ...
          {loadingSubscription ? (
            <div className="p-3 bg-muted rounded-lg">
              <div className="animate-pulse text-muted-foreground">載入目前訂閱狀態...</div>
            </div>
          ) : currentSubscription ? (
          ...
          {/* 警告提示 - keeping yellow as functional warning color */}
