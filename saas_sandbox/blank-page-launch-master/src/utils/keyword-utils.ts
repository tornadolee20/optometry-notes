...
export const formatStoreKeywords = (storeKeywords: RawStoreKeyword[]): Keyword[] => {
  return storeKeywords.map(kw => ({
    id: kw.id,
    keyword: kw.keyword,
    category: (kw.category || 'general') as Keyword['category'],
    source: (kw.source || 'manual') as Keyword['source'],
