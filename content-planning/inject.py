...
# The iframe tag to inject
iframe_tag = f'\\n<iframe src="{data_uri}" width="100%" height="900" style="border:none; margin: 30px 0; border-radius: 20px; box-shadow: 0 10px 25px rgba(0,0,0,0.1);"></iframe>\\n'

file_path = "明明1.0卻看不清-症狀研究分析-完整版.html"

# Read the HTML file
with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# Replace existing content: specifically after the </ol> tag corresponding to CISS
target = "</ol>"
if target in content and iframe_tag not in content:
    # Find the FIRST </ol> in the file and insert the iframe right after it
    # We know the CISS list is the first and only <ol> in the file
    content = content.replace(target, target + iframe_tag, 1)
