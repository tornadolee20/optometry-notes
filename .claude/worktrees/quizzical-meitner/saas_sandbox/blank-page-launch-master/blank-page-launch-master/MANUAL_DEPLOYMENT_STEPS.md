# 🔧 手動部署Supabase函數 - 立即可行方案

## 📋 **您的專案信息**
- **Project ID**: `atblgdosaclzvfdwfqvj`
- **函數名稱**: `generate-review`

## 🚀 **立即部署步驟 (使用Dashboard)**

### **步驟1: 登入Supabase Dashboard**
1. 前往：https://supabase.com/dashboard
2. 選擇專案：`atblgdosaclzvfdwfqvj`

### **步驟2: 前往Edge Functions**
1. 在左側選單點擊 **"Edge Functions"**
2. 找到現有的 `generate-review` 函數
3. 點擊函數名稱進入編輯
...
serve(async (req) => {
  ...
  try {
    ...
    if (isNegativeMode) {
      // 負面評論模式：簡單直接的陳述事實風格
      systemPrompt = `請撰寫一則 **簡潔客觀的負面 Google 評論**：
...
**重要**：不要使用複雜的修辭，不要推薦朋友，直接陳述體驗問題即可。`;
      ...
      // 原本的正面/中性評論邏輯
      systemPrompt = `請根據以下條件，撰寫一則 **符合顧客親身體驗的 Google 評論**：
...
### **步驟6: 測試函數**
1. 回到應用程式測試評論生成功能
2. 查看函數日誌：**Edge Functions > generate-review > Logs**
...
1. 查看函數日誌中的詳細錯誤信息
2. 確認OpenAI API金鑰設定正確
3. 檢查網路連接

---

**修復關鍵點**: 主要修復了`generatedText`變數作用域問題，這是導致500錯誤的根本原因。